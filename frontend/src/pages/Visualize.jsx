...
function Visualize() {
  ...
  // 📦 Fetch from Chrome local storage
  const fetchFromChromeStorage = () => {
    return new Promise((resolve) => {
      ...
      chrome.storage.local.get("savedVisualizations", (res) => {
        resolve(res.savedVisualizations || []);
      });
    });
  };

  // ⚙️ Generate visualization + 💾 Save it
  const handleVisualize = async () => {
    if (!text.trim()) return alert("Please enter a topic!");
    setLoading(true);
    setVisualization("");

    try {
      const res = await fetch(`${BASE_URL}/visualize/generate`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text }), // only text now
      });
      if (!res.ok) throw new Error(`Server error: ${res.status}`);
      const data = await res.json();

      const result = data.visualization || "No visualization generated.";
      setVisualization(result);
      announce(`Visualization generated successfully`);

      // 💾 Save generated visualization to backend
      const saveRes = await fetch(`${BASE_URL}/visualize/save`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          text,
      ...
      if (!saveRes.ok) {
        console.warn("⚠️ Save request failed:", saveRes.status);
      } else {
        console.log("✅ Visualization saved to backend");
      }
      ...
      if (chrome?.storage?.local) {
        chrome.storage.local.get({ savedVisualizations: [] }, (res) => {
          ...
          chrome.storage.local.set({ savedVisualizations: updated }, () => {
            console.log("✅ Visualization saved to Chrome storage");
          });
        });
      }

      await fetchStored(); // refresh stored list
    } catch (err) {
      console.error("❌ Failed to generate visualization:", err);
      setVisualization("❌ Failed to generate visualization.");
      announce("Visualization generation failed");
    } finally {
      setLoading(false);
    }
  };

  // 📦 Fetch stored visualizations (backend + chrome)
  const fetchStored = async () => {
    try {
      const [backendRes, chromeData] = await Promise.allSettled([
        fetch(`${BASE_URL}/visualize/stored`).then((r) => r.json()),
        fetchFromChromeStorage(),
      ]);

      const backendData =
        backendRes.status === "fulfilled" ? backendRes.value.visualizations || [] : [];
      const chromeStored =
  ...
  const openInNewTab = (content, title = "Visualization") => {
    if (!content) return;
      ...
      <html lang="en">
        <head>
          ...
          <style>
            ...
            h2, h3, h4, strong, span { color: #0694d1; }
        ...
        <body>
          ...
          <pre class="container">${htmlContent}</pre>
        </body>
      </html>
    `);
  ...
  const recentTitle = extractTitle(visualization);
    ...
    <div className="w-[400px] h-[450px] overflow-hidden">
      <div className="w-full h-full bg-[#fcfbf1] dark:bg-[#323c4b] flex flex-col p-4 transition-all duration-300">
        ...
        {activeSection === "Recent" && (
          <div className="mt-5 flex flex-col items-center gap-3">
            <input
              ...
              className="w-full h-[40px] bg-transparent border-2 border-[#0694d1] rounded-full shadow-sm px-3 text-lg text-[#0694d1] dark:text-[#fcfbf1] focus:outline-none"
            />
            ...
            {visualization && (
              <div className="mt-5 h-[70px] bg-[#efede9] dark:bg-[#2a333f] rounded-lg shadow-md flex items-center justify-between overflow-hidden px-4 relative w-full">
                ...
                <div className="flex gap-3 text-sm">
                  <button onClick={() => handleExpand({ visualization })} className="h-7 linear text-white rounded-md p-1 font-bold">Expand</button>
                  <button onClick={() => openInNewTab(visualization, recentTitle)} className="h-7 linear text-white rounded-md p-1 font-bold">Open</button>
                </div>
              </div>
            )}
          </div>
        ...
        {activeSection === "Stored" && (
          <div className="storeTab mt-5 flex flex-col gap-3 overflow-auto h-[340px]">
            {stored.length === 0 ? (
              <p className="text-center text-[#0694d1]">No stored visualizations yet.</p>
            ) : (
              stored.map((item, idx) => {
                const title = extractTitle(item.visualization);
                return (
                  <div key={idx} className="bg-[#efede9] dark:bg-[#2a333f] rounded-lg shadow-md">
                    <div className="flex justify-between items-center relative h-[70px] w-full overflow-hidden">
                      ...
                      <div className="flex gap-3 text-sm mr-3">
                        <button onClick={() => handleExpand(item)} className="h-7 linear text-white rounded-md p-1 font-bold">Expand</button>
                        <button onClick={() => openInNewTab(item.visualization, title)} className="h-7 linear text-white rounded-md p-1 font-bold">Open</button>
                      </div>
                    </div>
                  </div>
                );
      ...
      {showModal && selectedItem && (
        <div
          ...
          style={{ backgroundColor: isDarkMode ? "#0f172aee" : "rgba(0,0,0,0.6)" }}
        >
          <div
          ...
          >
            <div className="flex justify-between items-center p-4 text-white linear rounded-t-2xl">
              <h2 className="text-xl font-semibold">Full Visualization View</h2>
              <div className="flex gap-3">
                <button onClick={() => handleZoom("out")} className="bg-white text-[#0694d1] px-2 rounded-full font-bold">−</button>
                <button onClick={() => handleZoom("in")} className="bg-white text-[#0694d1] px-2 rounded-full font-bold">+</button>
                <button onClick={() => handleZoom("reset")} className="bg-white text-[#0694d1] px-3 rounded-full text-sm font-semibold">Reset</button>
                <button onClick={() => setShowModal(false)} className="bg-red-600 text-white px-3 rounded-full text-sm font-semibold">Close</button>
              </div>
            </div>
            <div
              className="flex-1 p-6 overflow-auto prose dark:prose-invert storeTab"
              style={{
            ...
            >
              <pre>{selectedItem.visualization}</pre>
              <button onClick={() => copyToClipboard(selectedItem.visualization)} className="absolute right-4 top-0 rounded-full linear text-white px-3 py-1 mt-2 text-sm font-semibold z-10">
                {copied ? "Copied!" : "Copy"}
              </button>
